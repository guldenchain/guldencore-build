 ...
 * <ul>
 * <li> `browser:uncompressed` - build uncomprssed browser bundle (`guldencore-*.js`)
 * <li> `browser:compressed` - build compressed browser bundle (`guldencore-*.min.js`)
...
function startGulp(name, opts) {
  ...
  var fullname = name ? 'guldencore-' + name : 'guldencore';
  var files = ['lib/**/*.js'];
  var tests = ['test/**/*.js'];
  var alljs = files.concat(tests);

  var buildPath = './node_modules/guldencore-build/';
  ...
  if (browser) {
    ...
    if (name !== 'lib') {
      browserifyCommand = buildBinPath + 'browserify --require ./index.js:' + fullname + ' --external guldencore-lib -o ' + fullname + '.js';
    } else {
      browserifyCommand = buildBinPath + 'browserify --require ./index.js:guldencore-lib -o guldencore-lib.js';
    }
  ...
  var release = function(importance, cb) {
    ...
    return runsequence(
      ...
      // Run git push guldenchain $VERSION
      'release:push-tag',
      // Run npm publish
      'release:publish',
